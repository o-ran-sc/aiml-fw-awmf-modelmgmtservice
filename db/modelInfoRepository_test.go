...
// db_repository_subtests_test.go
package db
...
func openTestDB(t *testing.T) *gorm.DB {
	...
	if err := d.AutoMigrate(&models.ModelRelatedInformation{}); err != nil {
		t.Fatalf("automigrate: %v", err)
	}
	if err := d.Exec(`
		CREATE TABLE IF NOT EXISTS target_environments (
			model_related_information_id TEXT,
			platform_name TEXT,
...
func TestCRUD_Subtests(t *testing.T) {
	...
	t.Run("UpdateReplace", func(t *testing.T) {
	...
	t.Run("UpdatePartialNil", func(t *testing.T) {
		cur, err := repo.GetModelInfoById(id)
		if err != nil {
			t.Fatalf("get: %v", err)
		}
