...
func TestRegisterModelFailCreateDuplicateModel(t *testing.T) {
	os.Setenv("LOG_FILE_NAME", "testing")
	iDBMockInst := new(iDBMock)
	iDBMockInst.On("Create", mock.Anything).Return(fmt.Errorf("duplicate key value violates unique constraint \"model_related_informations_pkey\" (SQLSTATE 23505)"))
	handler := apis.NewMmeApiHandler(nil, iDBMockInst)
	router := routers.InitRouter(handler)
	w := httptest.NewRecorder()
	req, _ := http.NewRequest("POST", "/ai-ml-model-registration/v1/model-registrations", strings.NewReader(registerModelBody))
	...
	assert.Equal(t, `{"code":409,"title":"model name and version combination already present","detail":"The request json is not correct as , duplicate key value violates unique constraint \"model_related_informations_pkey\" (SQLSTATE 23505)"}`, string(body))
}

func TestRegisterModelFailCreate(t *testing.T) {
	os.Setenv("LOG_FILE_NAME", "testing")
	iDBMockInst := new(iDBMock)
	iDBMockInst.On("Create", mock.Anything).Return(fmt.Errorf("db not available"))
	handler := apis.NewMmeApiHandler(nil, iDBMockInst)
	router := routers.InitRouter(handler)
	w := httptest.NewRecorder()
	req, _ := http.NewRequest("POST", "/ai-ml-model-registration/v1/model-registrations", strings.NewReader(registerModelBody))
	...
	assert.Equal(t, `{"code":500,"title":"Internal Server Error","detail":"The model cannot be registered due to , db not available"}`, string(body))
}
