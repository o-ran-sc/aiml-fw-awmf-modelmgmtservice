...
func TestUploadModelFailureModelNotRegistered(t *testing.T) {
	...
	assert.Equal(
		t,
		fmt.Sprintf(`{"status":404,"title":"Model not registered","detail":"ModelName: %s and modelVersion: %s is not registered, Kindly register it first!"}`, modelName, modelVersion),
		string(responseBody),
	)
}
