...
type ModelInformation struct {
	...
	InputDataType	[]float64	`json:"inputdatatypes"`
	OutputDataType	[]float64	`json:"outputdatatypes"`
}

type ModelRelatedInformation struct {
	ModelId		ModelID                 `json:"model-id"`
	Description	string					`json:"description"`
	ModelInformation	ModelInformation	`json:"model-information"`
}
...
func (m *MmeApiHandler) RegisterModel(cont *gin.Context) {
	...
	var modelrelatedinfo ModelRelatedInformation
	//Need to unmarshal JSON to Struct, to access request
	//data such as model name, rapp id etc
	err := json.Unmarshal(bodyBytes, &modelrelatedinfo)
	if err != nil || modelrelatedinfo.ModelId.ModelName == "" {
		...
		logging.INFO(string(modelrelatedinfo))
		modelInfoBytes, _ := json.Marshal(modelrelatedinfo)
		modelName:= modelrelatedinfo.ModelId.ModelName
